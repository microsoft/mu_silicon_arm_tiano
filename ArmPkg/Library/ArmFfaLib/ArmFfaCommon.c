...
/**
  ...
  @param [in] FfaStatus          FF-A return code
...
/**
  Check FF-A result is error situation.
  ...
  )
{
   ...
   */
  if (IS_FID_FFA_ERROR (FfaArgs->Arg0)) {
    return TRUE;
  }
   ...
   */
  if (FfaArgs->Arg0 == ARM_FFA_RET_NOT_SUPPORTED) {
    FfaArgs->Arg2 = ARM_FFA_RET_NOT_SUPPORTED;
    return TRUE;
  }
